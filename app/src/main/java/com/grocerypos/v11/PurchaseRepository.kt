        ...
        return try {
            ...
            db.withTransaction {
                ...
                if (original != null) {
                    ...
                    db.purchaseDao().deleteItems(billNo)
                    db.purchaseDao().deletePurchase(billNo)
                    // FIX (phantom payment on edit): payment/cash-transaction records
                    // tied to this bill are NO LONGER unconditionally wiped here. They
                    // used to be deleted and then unconditionally recreated below with
                    // amount=amountPaid, which meant editing a purchase for ANY reason
                    // (even just fixing a qty, not touching payment) silently deleted
                    // real payment history and fabricated a brand-new payment stamped
                    // with today's date/time — a payment the user never actually made.
                    // The block below now only touches these records when amountPaid
                    // has actually changed from what's already on file.
                }
                ...
                if (supplierId != null) {
                    // FIX (phantom payment on edit): update the SAME payment record
                    // in place when the paid amount actually changed (keeps its
                    // original id/serverId/createdAt, so history & date stay real),
                    // only insert a genuinely NEW record when this bill had no
                    // payment before, only delete when paid amount was zeroed out,
                    // and touch nothing at all when the amount didn't change.
                    val existingPayment = if (original != null) db.paymentDao().findByReference(billNo) else null
                    when {
                        amountPaid <= 0.0 && existingPayment != null -> {
                            db.paymentDao().deleteByReference(billNo)
                            existingPayment.serverId?.let { SyncQueueHelper.enqueueDelete(db, "payment", it) }
                        }
                        amountPaid > 0.0 && existingPayment != null && existingPayment.amount != amountPaid -> {
                            val updated = existingPayment.copy(amount = amountPaid, method = paymentMethod, note = "Purchase payment (edited)")
                        ...
                        // else: amountPaid unchanged — leave the existing payment untouched.
                    }
                }
                run {
                    // FIX (phantom payment on edit): same in-place-update approach
                    // for the linked Cash Out entry, so Cash Book/Day Book don't get
                    // a re-dated duplicate every time a purchase is merely edited.
                    val existingCashTx = if (original != null) db.cashTransactionDao().findByReference(billNo) else null
                    when {
                        amountPaid <= 0.0 && existingCashTx != null -> {
                            db.cashTransactionDao().deleteByReference(billNo)
                            existingCashTx.serverId?.let { SyncQueueHelper.enqueueDelete(db, "cash_transaction", it) }
                        }
                        amountPaid > 0.0 && existingCashTx != null && existingCashTx.amount != amountPaid -> {
                            val updated = existingCashTx.copy(amount = amountPaid, method = paymentMethod.lowercase())
                            db.cashTransactionDao().update(updated)
                            SyncQueueHelper.enqueueCashTransaction(db, updated)
                        }
                        amountPaid > 0.0 && existingCashTx == null -> {
                        ...
                        // else: amountPaid unchanged — leave the existing cash entry untouched.
                    }
                }
