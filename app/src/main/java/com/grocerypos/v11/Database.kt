...
class SyncWorker(ctx: Context, params: WorkerParameters) : CoroutineWorker(ctx, params) {
    ...
    override suspend fun doWork(): Result = withContext(Dispatchers.IO) {
        ...
        for (entry in pending) {
            try {
                val ok = SyncApi.push(entry) // implement: POST payload to your backend
                if (ok) {
        ...
        try {
            ...
            settingDao.set(com.grocerypos.v11.AppSetting("last_sync_ts", System.currentTimeMillis().toString()))
        } catch (e: Exception) {
            anyFailed = true
        }
